...
fx5=-1/(2*dx5):1/x_range:1/(2*dx5)-1/x_range;  % calculate the FT coordinates
fy5=-1/(2*dy5):1/y_range:1/(2*dy5)-1/y_range;  % calculate the FT coordinates
...
fx6=-1/(2*dx6):1/xprime5_full_range:1/(2*dx6)-1/xprime5_full_range;  % calculate the FT coordinates
fy6=-1/(2*dy6):1/xprime5_full_range:1/(2*dy6)-1/xprime5_full_range;  % calculate the FT coordinates
...
z = 0;         %z_bef, position(in m) of the GB beam waist comparing with the position of the lens
t = 0;
...
alpha = 5 * pi / 180;             %axicon lens base angle 5 degrees
R_exp = 1.0 * 10 ^(-3);           %radius of curvature of the hyperbole
e0 = 6.1 * 10 ^ (-3);             %thickness of axicon at apex
...
[U15,U19] = propTF2dim_DoubleIdealAxicon(E_GB,X,Y,X_prime5,Y_prime5,X_prime6,Y_prime6,k,n,alpha,z4,z5);
...
function[U15,U19]=propTF2dim_DoubleIdealAxicon(u1,X_prime4,Y_prime4,X_prime5,Y_prime5,X_prime6,Y_prime6,k,n,alpha,z4,z5)
...
% propagation - transfer function approach
% Hyperbolic corrected Axicon Pair
% using fresnel approximation
% assumes equal side lengths for x and y
% uniform sampling
